...
export default function RenewApplicationPage() {
  const router = useRouter()
  const searchParams = useSearchParams()
  const [currentStep, setCurrentStep] = useState(1)

  // Initialize form data with URL parameters from the calculator
  const initialFormData = {
    ...
    // Mortgage Details
    currentMortgageBalance: searchParams.get("currentBalance") || "",
    currentInterestRate: searchParams.get("currentRate") || "",
    currentLender: "",
    maturityDate: "",

    // New Mortgage Details
    newInterestRate: searchParams.get("newRate") || "",
    newTerm: searchParams.get("term") || "",
    newPaymentFrequency: "monthly",

    // Renewal Savings (from calculator)
    currentMonthlyPayment: searchParams.get("currentMonthlyPayment") || "",
    newMonthlyPayment: searchParams.get("newMonthlyPayment") || "",
    monthlySavings: searchParams.get("monthlySavings") || "",
  ...
  const formatCurrency = (value: string | null) => {
    ...
    return new Intl.NumberFormat("en-CA", { style: "currency", currency: "CAD" }).format(numValue)
  }
  ...
  return (
    <Layout>
      <div className="container mx-auto px-4 py-12">
        ...
        <h1 className="text-3xl font-bold mb-6">Mortgage Renewal Application</h1>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => handleChange("currentLender", e.target.value)}
                      required
                    />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="maturityDate">Mortgage Maturity Date</Label>
                    <Input
                      id="maturityDate"
                      type="date"
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={formData.newTerm} onValueChange={(value) => handleChange("newTerm", value)}>
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="newPaymentFrequency">Payment Frequency</Label>
                    <Select
                      value={formData.newPaymentFrequency}
                      onValueChange={(value) => handleChange("newPaymentFrequency", value)}
                    >
                      <SelectTrigger id="newPaymentFrequency">
                ...
                {/* Display savings from calculator if available */}
                {(formData.currentMonthlyPayment || formData.newMonthlyPayment || formData.monthlySavings) && (
                  <div className="mt-6 p-4 bg-primary/5 rounded-lg">
                    <h3 className="text-lg font-semibold mb-2">Your Potential Savings</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <p className="text-sm text-gray-600">Current Monthly Payment</p>
                        <p className="font-semibold">{formatCurrency(formData.currentMonthlyPayment)}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-600">New Monthly Payment</p>
                        <p className="font-semibold">{formatCurrency(formData.newMonthlyPayment)}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-600">Monthly Savings</p>
                        <p className="font-semibold text-primary">{formatCurrency(formData.monthlySavings)}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-600">Total Savings Over Term</p>
                        <p className="font-semibold text-primary">{formatCurrency(formData.totalSavings)}</p>
                      </div>
                    </div>
                  </div>
                )}
                  ...
                  <TabsContent value="mortgage" className="p-4 border rounded-md mt-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <p>{formData.newTerm} years</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-500">Payment Frequency</p>
                        <p>{formData.newPaymentFrequency}</p>
                      </div>
                    </div>
                      ...
                      <div className="mt-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            ...
                            <p className="text-primary">{formatCurrency(formData.monthlySavings)}</p>
                          </div>
                          <div>
                            <p className="text-sm text-gray-500">Total Savings Over Term</p>
                            <p className="text-primary">{formatCurrency(formData.totalSavings)}</p>
                          </div>
                        </div>
                      </div>
                    )}
